...
def display_admin_view():
    ...
    clients = {"All": "All"}
    for company_id, company_name in get_companies().items():
        clients[company_id] = company_name
    client = st.selectbox("Client", clients.values())
    st.write(f"Selected client: {client}")
    month_list = []
    for i in range (36):
        month_list.append((datetime.date.today() - relativedelta(months=i)).strftime("%B %Y"))
